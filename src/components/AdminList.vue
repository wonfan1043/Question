...
<template>
    ...
    <div class="list">
        <div class="left">
            <br><br><br><br><br>
            <router-link class="link" to="/createCheck">
        ...
        <div class="right" ref="list">
            <div class="ques" :class="{quesMove: this.moveQues}" v-for="item in this.showList">
                ...
                <router-link class="router" to="/reviseCheck" v-if="item.status == '未發布' || item.status == '未開始'"  @click="this.sendQuizId(item.num)">
                    <div class="enter text btn">進入</div>
                </router-link>
                <router-link class="router" to="/adminResult" v-if="item.status == '進行中' || item.status == '已結束'"  @click="this.sendQuizId(item.num)">
