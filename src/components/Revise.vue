...
export default {
    data() {
        return {
            ...
            startDate: "",
            endDate: "",
            quesNum: [], //題號陣列，同時用來生成對應數量的題目框
            ques: [0], //題目陣列，index為題號，value為題目
            quesType: [0], //題型陣列，index為題號，value為題型
            isRequired: [0], //是否必填陣列，index為題號，value為是否必填
            choices: [0], //選項陣列，index為題號，value為選項
            mail: [] //用來emit的陣列
        }
    },
    components: {
    ...
    mounted() {
        ...
        fetch("http://localhost:8080/quiz/search_quiz", {
            ...
            .then((data) => {
                ...
                this.title = data.quizList[0].quiz_name;
                this.description = data.quizList[0].quiz_description;
                this.startDate = data.quizList[0].start_date;
                this.endDate = data.quizList[0].end_date;
                data.quizList.forEach(item => {
                    this.quesNum.push(item.question_id);
                    this.ques.push(item.question_name);
                    this.quesType.push(item.question_type);
                    this.isRequired.push(item.is_necessary);
                    this.choices.push(item.question_options);
                });
                console.log(this.title);
                console.log(this.description);
                console.log(this.quesNum);
